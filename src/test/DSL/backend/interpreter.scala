...
class InterpreterSpec extends AnyFlatSpec {
  ...
  "Interpreter" should "interpret IntLiteral as point mass" in {
    assertDist(IntLiteral(5))(5 -> 1.0)
  }

  it should "interpret 1d6 as uniform 1..6" in {
    val d = interpreter.interpret(Dice(IntLiteral(1), IntLiteral(6)))
    d.size shouldBe 6
  ...
  it should "interpret 2d6 with correct sum distribution" in {
    val d = interpreter.interpret(Dice(IntLiteral(2), IntLiteral(6)))
  ...
  it should "interpret Sum as identity on distribution" in {
    val expr = Sum(Dice(IntLiteral(2), IntLiteral(6)))
    interpreter.interpret(expr) shouldEqual interpreter.interpret(Dice(IntLiteral(2), IntLiteral(6)))
  }

  it should "interpret Add as convolution" in {
    // 1 + 2 = 3 with prob 1
    assertDist(Add(IntLiteral(1), IntLiteral(2)))(3 -> 1.0)
    // 1d1 + 1d1 = 2 with prob 1 (1+1)
    val d = interpreter.interpret(Add(Dice(IntLiteral(1), IntLiteral(1)), Dice(IntLiteral(1), IntLiteral(1))))
    d shouldEqual Map(2 -> 1.0)
  }

  it should "interpret mixed expression 2d6 + 5" in {
    val expr = Add(Dice(IntLiteral(2), IntLiteral(6)), IntLiteral(5))
